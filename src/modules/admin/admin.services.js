...
export const addCandidateExcel = async (req, res, next) => {
  try {
    ...
    return res.status(200).json({  message: "Candidates added successfully",  failedRows});
  } catch (error) {console.error("Error while uploading candidates:", error);
  ...
  try {
    ...
    for (const row of users) {
      ...
      try {
        ...
        const createUser = await userModel.create({ userName, email, password: hashedPassword, cardnumber, phone, address, gender, role,
          image: {
...
export const addCandidateExcel = async (req, res, next) => {
  try {
    ...
    const errors = [];
    const successes = [];
    const emails = []; // Array to hold valid emails
